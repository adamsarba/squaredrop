...
<article id="post-<?php the_ID(); ?>" <?php post_class( [ 'post-card' ] ); ?>>
  ...
  <?php if ( has_post_thumbnail() ): ?>
    <a class="card-media" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>" aria-hidden="true" tabindex="-1">
      <?php 
        ...
        // the_post_thumbnail( 'really-simple-thumb', [ 'class' => 'really-thumb' ] );
      ?>
    </a>
  ...
  <div class="card-body">
    ...
    <div class="entry-content">
      ...
      ?>
      <a href="<?php the_permalink(); ?>" rel="bookmark" class="more-link">
        <?php
          if ( 'product' === get_post_type() ) {
            _e('See', 'really-simple');
          } else {
            _e('Learn more', 'really-simple');
          }
        ?>
      &rarr;</a>
    </div><!-- .entry-content --> 
